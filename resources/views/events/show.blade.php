...
@section('title', $event->title . ' - Universepedia')
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="p-8">
            <div class="flex justify-between items-start mb-4">
                <div>
                    <span class="inline-block px-3 py-1 text-sm font-semibold text-purple-700 bg-purple-100 rounded-full mb-3">
                        {{ $event->getFormattedType() }}
                    </span>
                    <h1 class="text-4xl font-bold text-gray-800 mb-2">{{ $event->title }}</h1>
                    <p class="text-gray-500">📅 {{ $event->date->format('F d, Y') }}</p>
                    <p class="text-gray-400 text-sm mt-2">Created by: {{ $event->creator->name }}</p>
                    ...
                    <div class="rating-container">
                        ...
                        <span class="text-gray-600">{{ number_format($avg, 1) }} ({{ $event->totalRatings() }} ratings)</span>
                    </div>
                </div>
                ...
                <form action="{{ route('favorites.toggle') }}" method="POST" id="favoriteForm">
                    ...
                    <button 
                        type="submit" 
                        class="px-6 py-3 rounded-lg transition {{ auth()->user()->hasFavorited($event) ? 'bg-yellow-400 text-yellow-900 hover:bg-yellow-500' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}"
                        id="favoriteBtn"
                    >
                        {{ auth()->user()->hasFavorited($event) ? '★ Favorited' : '☆ Add to Favorites' }}
                    </button>
                </form>
            </div>

            <div class="border-t border-gray-200 pt-6 mt-6">
                <h2 class="text-2xl font-bold text-gray-800 mb-4">Rate This Event</h2>
            ...
            <div class="border-t border-gray-200 pt-6 mt-6">
                ...
                <div class="text-gray-700 leading-relaxed">{!! $event->description !!}</div>
            </div>
            ...
            @if(auth()->user()->isAdmin())
                <div class="border-t border-gray-200 pt-6 mt-6 flex gap-4">
                    ...
                    <form action="{{ route('events.destroy', $event) }}" method="POST" onsubmit="return confirm('This will soft delete. You can restore it later.')">
                        @csrf
                        @method('DELETE')
            ...
            <div class="comment-section">
                <h2 class="text-2xl font-bold text-gray-800 mb-4">Comments ({{ $event->comments->count() }})</h2>
                ...
                <div class="space-y-4">
                    @forelse($event->comments as $comment)
                        @include('partials.comment', ['comment' => $comment, 'type' => 'event', 'itemId' => $event->id])
                    @empty
